...
async function jsonRequest(url, key, body) {
...
async function optionalSource(configured, load) {
    if (!configured) return { status: 'not_connected', data: null };
    try { const data = await load(); return { status: 'connected', fetchedAt: new Date().toISOString(), data }; }
...
function createMarketingService({ supabase, env = process.env, refreshExcludedIds, isExcludedEmail }) {
    ...
    async function collect(days) {
        ...
        const [allProfiles, allDreams] = await Promise.all([
            readAll(() => supabase.from('profiles').select('id,created_at').order('id')),
            // No dream text, interpretation, names, or email addresses leave this service.
            readAll(() => supabase.from('dreams').select('id,user_id,created_at').order('id').lt('created_at', new Date(now).toISOString())),
        ]);
        ...
        const [posthog, revenuecat, web, email, billingCohorts] = await Promise.all([
            optionalSource(env.POSTHOG_PERSONAL_API_KEY, async () => {
                ...
                for (let i = 0; i < ids.length; i += 200) {
                    ...
                    const rows = await ph(`SELECT distinct_id, argMax(person.properties.acquisition_source, timestamp), argMax(person.properties.install_utm_source, timestamp) ${base} GROUP BY distinct_id LIMIT 201`);
                    attribution.push(...rows.map(([id, reported, observed]) => ({ id, reported, observed })));
                    const reaches = await ph(`SELECT event, uniqExact(distinct_id) ${base} AND timestamp >= toDateTime('${start.slice(0, 19).replace('T', ' ')}', 'UTC') AND event IN ('paywall viewed','paywall purchase started','paywall purchase succeeded','paywall purchase cancelled','paywall purchase failed') GROUP BY event LIMIT 10`);
            ...
            optionalSource(env.REVENUECAT_SECRET_API_KEY, async () => {
                const result = await jsonRequest(`https://api.revenuecat.com/v2/projects/${encodeURIComponent(env.REVENUECAT_PROJECT_ID || 'projb2ec85a8')}/metrics/overview?currency=USD`, env.REVENUECAT_SECRET_API_KEY);
            ...
            optionalSource(env.VERCEL_TOKEN && env.VERCEL_PROJECT_ID, async () => {
                ...
                const result = await jsonRequest(`https://api.vercel.com/v1/query/web-analytics/visits/count?${params}`, env.VERCEL_TOKEN);
            ...
            optionalSource(true, async () => {
                const rows = await readAll(() => supabase.from('email_events').select('id,email_id,event_type,recipient,occurred_at').gte('occurred_at', start).lt('occurred_at', end).order('id'));
                const counts = {};
            ...
            optionalSource(env.REVENUECAT_SECRET_API_KEY, async () => {
                ...
                for (const chart of ['trial_conversion_rate', 'actives_new']) {
                    if (!chartOptions.has(chart)) chartOptions.set(chart, await jsonRequest(`${base}/${chart}/options`, key));
                    ...
                    const result = await jsonRequest(`${base}/${chart}?${params}`, key);
    ...
    return async days => {
        ...
        const promise = collect(days).catch(error => { cache.delete(days); throw error; });
        cache.set(days, { at: Date.now(), promise });
        return promise;
    };
}
