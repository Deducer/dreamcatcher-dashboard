...
function organizeReports() {
    ...
    outcomes.innerHTML='<h3>App outcome reporting</h3><p class="growth-detail">Paid-start production / sandbox scope is unverified. RevenueCat totals remain in Reports, outside business headline totals.</p>';
...
function renderOverview() {
    ...
    <section class="overview-chart"><div class="overview-section-header"><div><h2>Website traffic over time</h2><p id="overview-comparison" class="overview-caption">${e(comparison)}</p></div><div class="acq-chart-controls"><label class="sr-only" for="overview-metric">Traffic metric</label><select id="overview-metric" onchange="trafficMetric=this.value;renderTrafficTrend()"><option value="visitors" ${trafficMetric==='visitors'?'selected':''}>Visitors</option><option value="pageviews" ${trafficMetric==='pageviews'?'selected':''}>Page views</option></select><label class="sr-only" for="overview-style">Chart style</label><select id="overview-style" onchange="trafficStyle=this.value;renderTrafficTrend()"><option value="bars" ${trafficStyle==='bars'?'selected':''}>Bars</option><option value="line" ${trafficStyle==='line'?'selected':''}>Line</option></select></div></div><div id="overview-traffic-trend"></div>
