...
(function(root) {
    ...
    function overview({acquisition, marketing, mobile, channels, stores:storeReports, today=false, metric='visitors', now=Date.now()}={}) {
        ...
        if(storeReports?.android?.status==='access_unavailable') gaps.push({title:'Android downloads need report access',detail:'Check the Play Console reporting permission.',target:'channel-setup'});
