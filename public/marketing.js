...
function renderMobileAttribution() {
    ...
    document.getElementById('mobile-attribution-content').innerHTML = `<section class="chart-card growth-section">
        ...
        <details class="acq-daily-values"><summary>Report coverage and refresh times</summary><div class="growth-table-wrap"><table class="growth-table"><thead><tr><th>App / report</th><th>Status</th><th>Checked (UTC)</th><th>Next refresh eligible (UTC)</th></tr></thead><tbody>${platformRows.flatMap(p=>p.reports.map(r=>`<tr><td>${growthEscape(p.label)} · ${growthEscape(r.name.replace(/_/g,' '))}</td><td>${growthEscape(r.quotaResetsAt ? 'Daily export limit · resets at 00:00 UTC' : statuses[r.status]||r.status)}</td><td>${growthEscape(utc(r.checkedAt))}</td><td>${growthEscape(utc(r.refreshAfter))}</td></tr>`)).join('')}</tbody></table></div></details>
